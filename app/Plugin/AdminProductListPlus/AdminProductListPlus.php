...
class AdminProductListPlus
{
    ...
    public function onAdminProductIndexTwig(TemplateEvent $event) {
        ...
        // 価格,在庫追加
        $view = 'AdminProductListPlus/Resource/template/admin/Product/price_stock.twig';
        ...
        if(Constant::VERSION != "3.0.9") {
            // 3.0.10 以降を対象
            // タグ情報追加
            $view = 'AdminProductListPlus/Resource/template/admin/Product/add_tag.twig';
            $insert = $TwigRenderService->readTwigFile($view);
            $TwigRenderService->twigInsert($search, $insert, 9);
        }

        // カテゴリを追加
        $view = 'AdminProductListPlus/Resource/template/admin/Product/add_category.twig';
        $insert = $TwigRenderService->readTwigFile($view);
        $TwigRenderService->twigInsert($search, $insert, 9);
